...
parser.add_argument('--network_type',              type=str,   help='network type',      default='CVFT_LPN')
...
def compute_multi_loss(sat_global, grd_global):
    batch, dim, block = sat_global.get_shape().as_list()
    ...
    with tf.Session(config=config) as sess:
        ...
        if start_epoch == 0:
            if 'CVFT_LPN' in network_type:
                ...
                load_model_path = '~/CVFT_LPN/Model/Initialize/initial_model.ckpt'
        ...
        for epoch in range(start_epoch, start_epoch + number_of_epoch):
            ...
            file = '~/CVFT_LPN/Result/' + data_type + '/' + str(network_type) + '_accuracy.txt'
